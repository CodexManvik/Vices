...
import { Send, ImagePlus, Sparkle, Mic, X, Camera } from "lucide-react";
  ...
  return parts.map((part, index) => {
    ...
    if (isAi && part.startsWith('[TRIGGER_SELFIE:')) {
       if (mediaUrls && mediaUrls.length > 0) {
          ...
          return <span key={index} className="inline-flex items-center gap-2 px-3 py-1.5 mx-1 rounded-lg bg-[#c8324a]/20 border border-[#c8324a]/40 text-[#e87a8c] text-xs font-medium animate-pulse shadow-[0_0_15px_rgba(200,50,74,0.3)]"><Camera className="w-3.5 h-3.5" />taking a photo...</span>;
       }
    }
...
export default function App() {
  ...
  // Voice Toggle Mode Settings
  const [voiceEnabled, setVoiceEnabled] = useState(false);
  const recognitionRef = useRef<any>(null);
  const textBeforeRecordRef = useRef("");

  // Keep tracking sync blocks accurate across processing threads
  ...
  async function submitCorrection(originalInput: string, badResponse: string) {
    if (!correctionText.trim()) return;
    try {
      await fetch(`${apiBaseUrl}/feedback`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "ngrok-skip-browser-warning": "bypass" },
        body: JSON.stringify({ user_input: originalInput, rejected_response: badResponse, chosen_response: correctionText })
      });
  ...
  useEffect(() => {
    if (voiceEnabled && !typing) {
      ...
      textBeforeRecordRef.current = message;
      ...
      recognition.onresult = (event: any) => {
        ...
        const updatedText = (textBeforeRecordRef.current + " " + finalTranscript + " " + interimTranscript).trim();
        setMessage(updatedText);

        if (finalTranscript) {
          textBeforeRecordRef.current = (textBeforeRecordRef.current + " " + finalTranscript).trim();
        }
      };

      recognition.onerror = (event: any) => {
        if (event.error === 'not-allowed') {
          alert("Microphone access blocked. Enable browser mic permissions.");
          setVoiceEnabled(false);
        }
        console.error("Speech parsing interrupt event: ", event.error);
      };
    ...
    try {
      ...
      const processChunk = (rawText: string) => {
        ...
        while ((match = attachmentRegex.exec(cleanText)) !== null) {
          ...
          const imageUrl = `${apiBaseUrl}/images/${fileUrl}`;
          setMessages((prev: Msg[]) =>
            prev.map((x: Msg) => x.id === assistantId ? { ...x, mediaUrls: [...(x.mediaUrls || []), imageUrl] } : x)
          );
        ...
        while ((audioMatch = audioRegex.exec(cleanText)) !== null) {
          ...
          const audioUrl = `${apiBaseUrl}/images/${fileUrl}`;
          const audio = new Audio(audioUrl);
          audio.play().catch(e => console.log("Audio auto-playback deferred: ", e));
          cleanText = cleanText.replace(audioMatch[0], "");
        }

        if (cleanText) appendToAssistant(assistantId, cleanText);
      };
      ...
      setErrorText("The local core is currently offline or unlinked.");
    } finally {
      setTyping(false);
    }
  }
        ...
        <div className="w-full max-w-2xl rounded-3xl border border-white/[0.08] bg-black/40 backdrop-blur-2xl shadow-[0_30px_80px_rgba(0,0,0,0.6)] p-6 md:p-8">
          <div className="flex flex-col md:flex-row items-start justify-between gap-6">
            <div>
              ...
              <h1 className="mt-2 text-neutral-100" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: "2rem" }}>Completely Private & Open Development</h1>
  ...
  return (
    <div className="dark relative h-[100dvh] w-full overflow-hidden bg-[#070406] text-neutral-200">
      <div className="pointer-events-none absolute inset-0 will-change-transform">
        <motion.div className="absolute -top-32 -left-32 w-[300px] md:w-[520px] h-[300px] md:h-[520px] rounded-full" style={{ background: "radial-gradient(circle, rgba(140,20,40,0.20), transparent 60%)" }} animate={{ x: [0, 20, 0], y: [0, 10, 0] }} transition={{ duration: 25, repeat: Infinity, ease: "linear" }} />
        <motion.div className="absolute -bottom-40 -right-32 w-[400px] md:w-[600px] h-[400px] md:h-[600px] rounded-full" style={{ background: "radial-gradient(circle, rgba(60,10,80,0.25), transparent 60%)" }} animate={{ x: [0, -20, 0], y: [0, -10, 0] }} transition={{ duration: 30, repeat: Infinity, ease: "linear" }} />
      ...
      <div className="relative h-full flex flex-col max-w-7xl mx-auto">
        <motion.nav initial={{ y: -30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="flex items-center justify-between px-4 md:px-10 py-3 md:py-5 border-b border-white/[0.04] backdrop-blur-md bg-black/40 z-10">
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-8 h-8 md:w-9 md:h-9 rounded-lg bg-gradient-to-br from-[#3a0a18] to-[#1a040a] border border-[#5a1828]/40 flex items-center justify-center">
            ...
            <div className="relative h-full flex flex-col p-4 md:p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-neutral-100 mt-1" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300, fontSize: "1.75rem" }}>Rosia</h2>
                </div>
                <div className="px-3 py-1 rounded-full border border-[#c8324a]/30 bg-[#c8324a]/10 text-[10px] uppercase tracking-[0.2em] text-[#e87a8c]">alive</div>
              </div>
              <div className="relative aspect-square w-full md:max-w-none max-w-[280px] mx-auto rounded-2xl overflow-hidden bg-black">
                <Avatar />
